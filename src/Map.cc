...
void Map::ApplyScaledRotation(const Sophus::SE3f &T, const float s, const bool bScaledVel)
{
    ...
    {
        KeyFrame *pKF = *sit;
        Sophus::SE3f Twc(Converter::toMatrix4f(pKF->GetPoseInverse()));
        Twc.translation() *= s;
        ...
        pKF->SetPose(Converter::toCvMat(Tcy.matrix()));
    ...
    {
        MapPoint *pMP = *sit;
        Eigen::Vector3f posi = s * Ryw * Converter::toVector3f(pMP->GetWorldPos()) + tyw;
        pMP->SetWorldPos(Converter::toCvMat(posi));
