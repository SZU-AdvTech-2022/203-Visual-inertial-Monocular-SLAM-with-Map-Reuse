...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mbToBeErased(false), mbBad(false), mHalfBaseline(F.mb/2), mpMap(pMap),mpImuPreintegrated(F.mpImuPreintegrated),
    mImuCalib(F.mImuCalib),mbHasVelocity(false),mPrevKF(NULL), mNextKF(NULL)
{
    mnId=nNextId++;
...
void KeyFrame::SetPose(const cv::Mat &Tcw_)
{
    ...
    {
        mOwb = Converter::toMatrix3f(Rwc) * mImuCalib.mTcb.translation() + Converter::toVector3f(Twc.rowRange(0,3).col(3));
    }
}
...
{
    unique_lock<mutex> lock(mMutexPose);
    Sophus::SE3f setwc(Converter::toMatrix4f(Twc));
    return (setwc* mImuCalib.mTcb).rotationMatrix();
}
