...
ImuCamPose::ImuCamPose(KeyFrame *pKF):its(0)
{
    ...
    // Left camera
    tcw[0] = Converter::toVector3f(pKF->GetTranslation()).cast<double>();
    Rcw[0] = Converter::toMatrix3f(pKF->GetRotation()).cast<double>();
...
Eigen::Matrix<double, 2, 3> ImuCamPose::projectJac (const Eigen::Vector3d &v3D)
{
...
void EdgeMono::linearizeOplus()
{
    ...
    Eigen::Matrix<double, 2, 3> Jac;
    Jac(0, 0) = VPose->estimate().fx / Xc[2];
    Jac(0, 1) = 0.f;
    Jac(0, 2) = -VPose->estimate().fx * Xc[0] / (Xc[2] * Xc[2]);
    Jac(1, 0) = 0.f;
    Jac(1, 1) = VPose->estimate().fy / Xc[2];
    Jac(1, 2) = -VPose->estimate().fy * Xc[1] / (Xc[2] * Xc[2]);

    const Eigen::Matrix<double,2,3> proj_jac = Jac;
...
void EdgeMonoOnlyPose::linearizeOplus()
{
    ...
    Eigen::Matrix<double,2,3> proj_jac = Jac;
