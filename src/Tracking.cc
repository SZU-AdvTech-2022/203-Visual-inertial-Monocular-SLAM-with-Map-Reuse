...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Map *pMap, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor):
    ...
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0),mbMapUpdated(false)
{
    ...
    cv::Mat cvTbc = cv::Mat::eye(4,4,CV_32F);//x
    cvTbc.at<float>(0,0) = 0.0148655429818;
    cvTbc.at<float>(0,1) = -0.999880929698;
    cvTbc.at<float>(0,2) = 0.00414029679422;
    cvTbc.at<float>(0,3) = -0.0216401454975;

    cvTbc.at<float>(1,0) = 0.999557249008;
    cvTbc.at<float>(1,1) = 0.0149672133247;
    cvTbc.at<float>(1,2) = 0.025715529948;
    cvTbc.at<float>(1,3) = -0.064676986768;

    cvTbc.at<float>(2,0) = -0.0257744366974;
    cvTbc.at<float>(2,1) = 0.00375618835797;
    cvTbc.at<float>(2,2) = 0.999660727178;
    cvTbc.at<float>(2,3) = 0.00981073058949;

    cvTbc.at<float>(3,0) = 0.0;
    cvTbc.at<float>(3,1) = 0.0;
    cvTbc.at<float>(3,2) = 0.0;
    cvTbc.at<float>(3,3) = 1.0;

    Sophus::SE3f Tbc = Converter::toSophus(cvTbc);
    
    mImuFreq = fSettings["IMU.Frequency"];
    mImuPer = 0.001;
    float Ng = fSettings["IMU.NoiseGyro"];
    float Na = fSettings["IMU.NoiseAcc"];
    float Ngw = fSettings["IMU.GyroWalk"];
    float Naw = fSettings["IMU.AccWalk"];
...
void Tracking::PreintegrateIMU(){
    ...
    if(n==0){
        cout << "Empty IMU measurements vector" <<endl;
        return;   
    }
    ...
    //cout<< mCurrentFrame.mpImuPreintegratedFrame->dP << endl;
    //cout<< mCurrentFrame.mpImuPreintegratedFrame->dR << endl;
    //}
...
void Tracking::UpdateFrameIMU(const float s, const IMU::Bias &b, KeyFrame* pCurrentKeyFrame)
{
    ...
    for(auto lit=mlRelativeFramePoses.begin(),lend=mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lbL++)
    {
        ...
        Sophus::SE3f trcc(Converter::toMatrix4f(*lit));
        trcc.translation() *= s;
        (*lit) = Converter::toCvMat(trcc.matrix());
    }
    mLastBias = b;
    ...
    else
    {
        ...
        cout << "mLastFrame" << mLastFrame.GetVelocity() << endl;
    }
    ...
    if (mCurrentFrame.mpImuPreintegrated)
    {
        ...
        cout << "mCurrentFrame" << mCurrentFrame.GetVelocity() << endl;
    }
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                if(!mbVO)
                {
                    ...
                    if(!mVelocity.empty() )//&& mpMap->isImuInitialized())
                    {
                        bOK = TrackWithMotionModel();
                    }
                    else
        ...
        if(bOK)
        {
            ...
            if(mpLastKeyFrame&& mpMap->isImuInitialized()){
                const Eigen::Matrix3f Rwb1 = mpLastKeyFrame->GetImuRotation();
                const Eigen::Vector3f Vwb1 = mpLastKeyFrame->GetVelocity();
                ...
                Eigen::Vector3f Vwb2 = Vwb1 + t12*Gz + Rwb1 * mpImuPreintegratedFromLastKF->GetDeltaVelocity(mpLastKeyFrame->GetImuBias());
                mCurrentFrame.SetVelocity(Vwb2);
                //cout << mCurrentFrame.mnId <<  ": " << mCurrentFrame.GetVelocity() << endl;
            }
            */
...
bool Tracking::TrackWithMotionModel()
{
    ...
    //else
    //{
        mCurrentFrame.SetPose(mVelocity * Converter::toCvMat(mLastFrame.GetPose().matrix()));
    //}
...
{
    if(mpLocalMapper->bInitializing){
        return false;
    }
    if(mbOnlyTracking)
        return false;
